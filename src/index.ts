...
function initReorder(){list.querySelectorAll('.drag-handle').forEach(handle=>{let active=false,moved=false,row=null,pointerId=null,startX=0,startY=0;handle.addEventListener('pointerdown',event=>{if(event.button!==0)return;active=true;moved=false;row=handle.closest('.item');pointerId=event.pointerId;startX=event.clientX;startY=event.clientY;row.classList.add('sorting');document.body.classList.add('reordering');try{handle.setPointerCapture(pointerId)}catch(error){}});handle.addEventListener('pointermove',event=>{if(!active)return;event.preventDefault();if(!moved&&Math.hypot(event.clientX-startX,event.clientY-startY)<5)return;moved=true;moveRow(row,event.clientY)});const finish=async()=>{if(!active)return;active=false;row.classList.remove('sorting');document.body.classList.remove('reordering');try{if(handle.hasPointerCapture(pointerId))handle.releasePointerCapture(pointerId)}catch(error){}if(moved)await saveOrder()};handle.addEventListener('pointerup',finish);handle.addEventListener('pointercancel',finish);handle.addEventListener('keydown',async event=>{if(event.key!=='ArrowUp'&&event.key!=='ArrowDown')return;event.preventDefault();const rows=[...list.querySelectorAll('.item')],currentRow=handle.closest('.item'),index=rows.indexOf(currentRow);if(event.key==='ArrowUp'&&index>0)list.insertBefore(currentRow,rows[index-1]);else if(event.key==='ArrowDown'&&index<rows.length-1)list.insertBefore(rows[index+1],currentRow);else return;await saveOrder()})})}
