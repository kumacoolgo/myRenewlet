...
.ambient{z-index:0;background:#4c6f96}.wrap{position:relative;z-index:1}.ambient::after{background:rgba(5,31,66,.08)}.shape.one{left:-8vw;top:-15vh;opacity:.88}.shape.two{width:52vw;height:72vh;right:-7vw;top:-4vh;opacity:.78}.shape.three{left:-6vw;bottom:-8vh;opacity:.78}.mobile-label{display:none}@media(max-width:800px){.date-cell{grid-column:1/-1;display:flex;align-items:center;gap:8px;margin-top:0}.date-cell .mobile-label{display:inline;color:rgba(225,239,255,.48);font-size:11px;font-weight:700}}
.top{margin-bottom:16px}.stats{grid-template-columns:repeat(3,1fr)}.row-actions{align-items:center}.drag-handle{width:34px;height:34px;display:grid;place-items:center;flex:none;border:1px solid transparent;border-radius:10px;padding:0;background:transparent;color:rgba(207,226,255,.78);cursor:grab;touch-action:none;-webkit-user-select:none;user-select:none}.drag-handle:hover,.drag-handle:focus-visible{border-color:rgba(255,255,255,.28);background:rgba(255,255,255,.1);color:#fff;outline:none}.drag-handle svg{width:16px;height:20px;fill:currentColor;pointer-events:none}.drag-handle:active,.item.sorting .drag-handle{cursor:grabbing}.item.sorting{position:relative;z-index:3;transform:scale(1.01);background:rgba(225,240,255,.2);box-shadow:inset 4px 0 0 rgba(126,211,255,.92),inset 0 0 0 1px rgba(255,255,255,.46),0 16px 36px rgba(0,18,52,.34)}body.reordering{-webkit-user-select:none;user-select:none;cursor:grabbing;overscroll-behavior:none}@media(max-width:800px){.top{margin-bottom:14px}.stats{grid-template-columns:1fr 1fr}.stat:first-child{grid-column:1/-1;border-bottom:1px solid rgba(255,255,255,.2)}.stat:first-child::after{display:none}.stat:nth-child(2){border-bottom:0}.stat:nth-child(2)::after{display:block}.row-actions{gap:1px}.iconbtn{padding-left:7px;padding-right:7px}}@media(prefers-reduced-motion:reduce){.item{transition:none}}
...
<div class="top"><div class="brand"><div class="brand-mark">M</div><div><h1>MyBills</h1><p>家庭固定支出 · 保险 · 保修 · 服务生命周期</p></div></div><div class="top-actions"><form method="post" action="/logout"><button class="btn secondary" type="submit">退出</button></form><button class="btn" onclick="openForm()">＋ 新增项目</button></div></div>
<div class="stats"><div class="stat"><small>本月固定费用</small><b id="monthly">¥0</b></div><div class="stat"><small>30 天内到期</small><b id="due30">0</b></div><div class="stat"><small>保修即将到期</small><b id="warrantySoon">0</b></div></div>
<div class="surface"><div class="filters" id="filters"></div><div class="list-head"><span>项目名称</span><span>费用</span><span>到期提醒</span><span>结束 / 到期日</span><span>操作</span></div><div id="list"><div class="empty">载入中...</div></div></div></div>
...
<div class="field"><label>开始日期</label><input id="start_date" type="date"></div><div class="field"><label>结束 / 到期日</label><input id="end_date" type="date"></div>
...
function render(){const active=items.filter(x=>x.status==='active');const monthly=active.reduce((s,x)=>s+(x.billing_cycle==='monthly'?+x.amount||0:0),0);const due=active.filter(x=>{const d=days(x.end_date||x.next_renewal_date);return d!==null&&d>=0&&d<=30}).length;const ws=active.filter(x=>x.category==='warranty'&&(()=>{const d=days(x.end_date);return d!==null&&d>=0&&d<=x.reminder_days})()).length;monthlyEl.textContent=yen(monthly);due30El.textContent=due;warrantySoonEl.textContent=ws;
filters.innerHTML=Object.entries(cats).map(([k,v])=>'<button class="chip '+(current===k?'active':'')+'" data-category="'+k+'">'+v[1]+' '+v[0]+'</button>').join('');filters.querySelectorAll('.chip').forEach(button=>button.addEventListener('click',()=>{current=button.dataset.category;render()}));let rows=items.filter(x=>current==='all'||x.category===current);list.innerHTML=rows.length?rows.map(row).join(''):'<div class="empty">暂无项目</div>';initReorder()}
function row(x){const c=cats[x.category],d=days(x.end_date||x.next_renewal_date);let status='';if(d!==null){status=d<0?'<span class="danger">已过期 '+(-d)+' 天</span>':d<=x.reminder_days?'<span class="warn">剩余 '+d+' 天</span>':'<span class="ok">剩余 '+d+' 天</span>'}else status='<span class="muted">未设置到期日</span>';let cost=x.amount?(x.billing_cycle==='monthly'?yen(x.amount)+'/月':x.billing_cycle==='yearly'?yen(x.amount)+'/年':yen(x.amount)):'—';return '<div class="item" data-id="'+x.id+'"><div><div class="name"><span class="category-icon">'+c[1]+'</span><span>'+esc(x.name)+'</span></div><div class="muted">'+esc([x.owner,x.provider].filter(Boolean).join(' · '))+'</div></div><div class="money">'+cost+'</div><div class="hide-sm">'+status+'</div><div class="muted date-cell"><span class="mobile-label">结束 / 到期日</span><span>'+(x.end_date||x.next_renewal_date||'—')+'</span></div><div class="row-actions"><button class="iconbtn" aria-label="编辑 '+esc(x.name)+'" onclick="edit('+x.id+')">编辑</button><button class="iconbtn delete" aria-label="删除 '+esc(x.name)+'" onclick="del('+x.id+')">删除</button><button class="drag-handle" type="button" aria-label="按住并上下拖动排序 '+esc(x.name)+'" title="按住并上下拖动排序"><svg viewBox="0 0 16 20" aria-hidden="true"><circle cx="5" cy="4" r="1.5"/><circle cx="11" cy="4" r="1.5"/><circle cx="5" cy="10" r="1.5"/><circle cx="11" cy="10" r="1.5"/><circle cx="5" cy="16" r="1.5"/><circle cx="11" cy="16" r="1.5"/></svg></button></div></div>'}
function moveRow(row,clientY){const candidates=[...list.querySelectorAll('.item:not(.sorting)')];const before=candidates.find(candidate=>{const rect=candidate.getBoundingClientRect();return clientY<rect.top+rect.height/2});if(before)list.insertBefore(row,before);else list.appendChild(row)}
async function saveOrder(){const visibleIds=[...list.querySelectorAll('.item')].map(element=>Number(element.dataset.id));let visibleIndex=0;const ids=items.map(item=>(current==='all'||item.category===current)?visibleIds[visibleIndex++]:item.id);list.setAttribute('aria-busy','true');try{const response=await fetch('/api/items/reorder',{method:'PUT',headers:{'content-type':'application/json'},body:JSON.stringify({ids})});if(!response.ok)throw new Error('save failed');const byId=new Map(items.map(item=>[item.id,item]));items=ids.map(id=>byId.get(id));render()}catch(error){await load();alert('排序保存失败，请重试。')}}
function initReorder(){list.querySelectorAll('.drag-handle').forEach(handle=>{handle.addEventListener('pointerdown',event=>{if(event.pointerType==='mouse'&&event.button!==0)return;event.preventDefault();const row=handle.closest('.item'),pointerId=event.pointerId,startY=event.clientY;let lastY=startY,moved=false,orderChanged=false;row.classList.add('sorting');document.body.classList.add('reordering');try{handle.setPointerCapture(pointerId)}catch(error){}const onMove=event=>{if(event.pointerId!==pointerId)return;event.preventDefault();if(!moved&&Math.abs(event.clientY-startY)<4)return;moved=true;const direction=event.clientY>=lastY?1:-1;lastY=event.clientY;const before=[...list.querySelectorAll('.item')].map(element=>element.dataset.id).join(',');const target=[...list.querySelectorAll('.item:not(.sorting)')].find(candidate=>{const rect=candidate.getBoundingClientRect();return event.clientY>=rect.top&&event.clientY<=rect.bottom});if(target){if(direction>0)target.after(row);else target.before(row)}else moveRow(row,event.clientY);const after=[...list.querySelectorAll('.item')].map(element=>element.dataset.id).join(',');if(before!==after)orderChanged=true;if(event.clientY<72)window.scrollBy(0,-12);else if(event.clientY>window.innerHeight-72)window.scrollBy(0,12)};const finish=async event=>{if(event.pointerId!==pointerId)return;window.removeEventListener('pointermove',onMove,true);window.removeEventListener('pointerup',finish,true);window.removeEventListener('pointercancel',finish,true);row.classList.remove('sorting');document.body.classList.remove('reordering');try{if(handle.hasPointerCapture(pointerId))handle.releasePointerCapture(pointerId)}catch(error){}if(orderChanged)await saveOrder()};window.addEventListener('pointermove',onMove,{capture:true,passive:false});window.addEventListener('pointerup',finish,{capture:true});window.addEventListener('pointercancel',finish,{capture:true})});handle.addEventListener('keydown',async event=>{if(event.key!=='ArrowUp'&&event.key!=='ArrowDown')return;event.preventDefault();const rows=[...list.querySelectorAll('.item')],currentRow=handle.closest('.item'),index=rows.indexOf(currentRow);if(event.key==='ArrowUp'&&index>0)list.insertBefore(currentRow,rows[index-1]);else if(event.key==='ArrowDown'&&index<rows.length-1)list.insertBefore(rows[index+1],currentRow);else return;await saveOrder()})})}
...
const monthlyEl=document.getElementById('monthly'),due30El=document.getElementById('due30'),warrantySoonEl=document.getElementById('warrantySoon');load();
</script></body></html>`;
}
...
async function api(request: Request, env: Env, path: string) {
  if (path === '/api/items' && request.method === 'GET') {
    const result = await env.DB.prepare('SELECT * FROM household_items ORDER BY sort_order ASC, id ASC').all();
  ...
  if (path === '/api/items' && request.method === 'POST') {
    ...
    const result = await env.DB.prepare(
      `INSERT INTO household_items (${keys.join(',')},sort_order) VALUES (${marks},(SELECT COALESCE(MAX(sort_order),0)+1 FROM household_items))`,
    ).bind(...vals).run();
    return json({ id: result.meta.last_row_id }, 201);
  }
  if (path === '/api/items/reorder' && request.method === 'PUT') {
    const body = await request.json<Record<string, unknown>>();
    if (!Array.isArray(body.ids) || body.ids.length === 0 || body.ids.length > 500) {
      return json({ error: 'ids must be a non-empty array with at most 500 entries' }, 400);
    }
    const ids: number[] = [];
    for (const value of body.ids) {
      if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 1) {
        return json({ error: 'each id must be a positive integer' }, 400);
      }
      ids.push(value);
    }
    if (new Set(ids).size !== ids.length) return json({ error: 'duplicate ids are not allowed' }, 400);

    const existing = await env.DB.prepare('SELECT id FROM household_items ORDER BY id').all<{ id: number }>();
    const existingIds = new Set(existing.results.map(row => row.id));
    if (ids.length !== existingIds.size || ids.some(id => !existingIds.has(id))) {
      return json({ error: 'ids must include every current item exactly once' }, 409);
    }

    const updateOrder = env.DB.prepare(
      'UPDATE household_items SET sort_order=?, updated_at=CURRENT_TIMESTAMP WHERE id=?',
    );
    await env.DB.batch(ids.map((id, index) => updateOrder.bind(index + 1, id)));
    return json({ ok: true });
  }
